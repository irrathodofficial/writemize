...
final class RadarAgent
{
    ...
    public function run(array $context, array &$logs): array
    {
        ...
        $topicIdeas = [
            'AI Blog Automation Trends Businesses Should Watch in ' . date('Y'),
        ...
        return $this->client->json(
            ...
            'Using this business context, act like an SEO strategist and competitor analyst. Choose one fresh, viral, SEO-friendly blog topic for this business for today (' . date('Y-m-d') . '). The topic must be meaningfully different from previous posts and must not reuse these titles: ' . json_encode($recentTitles) . '. Use run_seed "' . $runSeed . '" to vary the angle. Prefer timely, high-intent, comparison, trend, checklist, how-to, or buyer-problem topics. Include keys: topic, focus_keyword, search_intent, angle, related_keywords, competitor_angles. Context: ' . json_encode($context),
            $fallback
        );
    }
}
